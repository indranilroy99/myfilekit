...
[![Version](https://img.shields.io/badge/version-3.20.0-2563eb)](./package.json)
...
| `pkijs` + `asn1js` (npm) | Digital Signature, Verify Signature | Builds and reads the PKCS#7/CMS `SignedData` for real PDF signing and verification on top of WebCrypto, so it runs identically in Node and the browser with no native crypto. `node-forge` was evaluated for this and rejected: it carries seven high-severity advisories with no fixed version, which would break the zero-vulnerability audit gate. `pkijs` and `asn1js` are audit-clean. |
...
- **No background removal and no OCR-based text replacement inside an existing image.** OCR reads text and can build a searchable layer over the original page; it does not rewrite the pixels of the page. Editing existing PDF text is now supported as an overlay (see below), not as a raster edit of an image.
...
- **Rasterising tools discard the original selectable text.** Compress PDF, Flatten PDF, Invert PDF Colours, Redact PDF, Deskew / Straighten, the PDF/A raster mode, and the searchable PDF that OCR produces all rebuild pages as images. That is what makes redaction permanent. The OCR output adds an invisible recognised-text layer back on top, so it is searchable again, but that text is the OCR engine's reading of the page rather than the source file's own. Compare PDFs does a text diff plus an optional visual pixel diff; the visual diff rasterises both files in the browser to tint the changed regions and is a comparison view, not an edit.
...
The current version is `3.20.0`. See [CHANGELOG.md](./CHANGELOG.md) and use the repository scripts to create intentional releases:
